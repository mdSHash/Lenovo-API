...
if type_info and type_info["TypeNumber"]:
    ...
    csv_filename = f'Warranty_and_Parts_info_{sanitized_serial}.csv'  # Create a CSV filename
    ...
    with open(csv_filename, mode='w', newline='', encoding='utf-8') as csvfile:
        ...
        csv_writer.writerow([])  # Add an empty row before part information
        csv_writer.writerow(["Part ID", "Part Name", "Substitutes", "Commodity Value", "Image URLs"])  # Header for parts
        
        parts_data = fetch_parts_data(serial_number, machine_type)

        # Filter and write unique part information
        unique_parts = {}
        for item in parts_data:
            part_id = item.get('id')  # Adjust based on the actual key for ID in your data
            if part_id not in unique_parts:
                unique_parts[part_id] = {
                    'id': part_id,
                    'name': item.get('name', 'N/A'),
                    'substitutes': item.get('substitutes', 'N/A'),
                    'commodityVal': item.get('commodityVal', 'N/A'),
                    'imageUrls': item.get('imageUrls', 'N/A')
                }

        for part in unique_parts.values():
            csv_writer.writerow([part['id'], part['name'], part['substitutes'], part['commodityVal'], part['imageUrls']])
else:
